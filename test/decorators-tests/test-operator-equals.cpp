...
TEST_CASE("Left transformation, both identical", "[TextTransformationDecorator]") {
    std::shared_ptr<LabelDecoratorBase> left_trim = std::make_shared<TextTransformationDecorator>(
        std::make_shared<SimpleLabel>("  hello  "), std::make_unique<LeftTrimTransformation>());
    std::shared_ptr<LabelDecoratorBase> left_trim2 = std::make_shared<TextTransformationDecorator>(
        std::make_shared<SimpleLabel>("  hello  "), std::make_unique<LeftTrimTransformation>());
...
TEST_CASE("Censor transformation, both identical", "[TextTransformationDecorator]") {
    std::shared_ptr<LabelDecoratorBase> censor = std::make_shared<TextTransformationDecorator>(
        std::make_shared<SimpleLabel>("hello"), std::make_unique<CensorTransformation>("hello"));
    std::shared_ptr<LabelDecoratorBase> censor2 = std::make_shared<TextTransformationDecorator>(
        std::make_shared<SimpleLabel>("hello"), std::make_unique<CensorTransformation>("hello"));
...
TEST_CASE("Censor transformation, different censor strings", "[TextTransformationDecorator]") {
    std::shared_ptr<LabelDecoratorBase> censor = std::make_shared<TextTransformationDecorator>(
        std::make_shared<SimpleLabel>("hello"), std::make_unique<CensorTransformation>("hello"));
    std::shared_ptr<LabelDecoratorBase> censor2 = std::make_shared<TextTransformationDecorator>(
        std::make_shared<SimpleLabel>("hello"), std::make_unique<CensorTransformation>("world"));
    
    REQUIRE_FALSE(*censor == *censor2);
}

TEST_CASE("Replace transformation, both identical", "[TextTransformationDecorator]") {
    std::shared_ptr<LabelDecoratorBase> replace = std::make_shared<TextTransformationDecorator>(
        std::make_shared<SimpleLabel>("hello"), std::make_unique<ReplaceTransformation>("hello", "world"));
    std::shared_ptr<LabelDecoratorBase> replace2 = std::make_shared<TextTransformationDecorator>(
        std::make_shared<SimpleLabel>("hello"), std::make_unique<ReplaceTransformation>("hello", "world"));
...
TEST_CASE("Replace transformation, different parameters", "[TextTransformationDecorator]") {
    std::shared_ptr<LabelDecoratorBase> replace = std::make_shared<TextTransformationDecorator>(
        std::make_shared<SimpleLabel>("hello"), std::make_unique<ReplaceTransformation>("hello", "world"));
    std::shared_ptr<LabelDecoratorBase> replace2 = std::make_shared<TextTransformationDecorator>(
        std::make_shared<SimpleLabel>("hello"), std::make_unique<ReplaceTransformation>("world", "hello"));
